...
| **A member lies about a third party's `lan_endpoint`**, e.g. pointing 256 peers' keepalives at one victim | Partly. For a `JOIN` arriving directly, the endpoint host is taken from the connection's observed source address and only the claimed port is kept — a node cannot lie about where it is. An observed source that is not an IPv4 LAN address rejects the `JOIN` outright (the listener is IPv4-only to begin with): an endpoint built from it would be a value every receiver's own validation rejects, poisoning each later `JOIN_REPLY` that carries it. Entries learned second-hand through `peers[]` have nothing to compare against and stay as claimed. | A member can still poison second-hand entries. Closing it needs per-node signatures, which is the certificate architecture of §12.4, not a field. |
